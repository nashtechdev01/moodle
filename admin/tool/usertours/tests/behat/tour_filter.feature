...
Feature: Apply tour filters to a tour
  ...
  Scenario: Check tabbing working correctly.
    ...
    And I wait "1" seconds
    And I press tab
    And the focussed element should be ".close" "css_element" in the "Customisation" "dialogue"
    And I press tab
    And the focussed element should be "Previous" "button" in the "Customisation" "dialogue"
    And I press tab
    And the focussed element should be "Next" "button" in the "Customisation" "dialogue"
    And I press tab
    And the focussed element should be "End tour" "button" in the "Customisation" "dialogue"
    # We tab 3 times from "End Tour" button to header container, drop down then go to "Dashboard" link.
    And I press tab
    And I press tab
    And I press tab
    And the focussed element should be "Dashboard" "link" in the ".breadcrumb" "css_element"
    And I press tab
    And the focussed element should be "Courses" "link"
    And I press tab
    And the focussed element should be "C1" "link"
