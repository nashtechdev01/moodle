...
if (typeof M.form.dependencyManager === 'undefined') {
    ...
    Y.extend(dependencyManager, Y.Base, {
        ...
        _disableElement: function(name, disabled) {
            ...
            if (editor) {
                var textareanode = Y.one('textarea[name="' + name + '[text]"]');
                if (typeof Y.M.editor_atto != 'undefined') {
                    var editordiv = textareanode.ancestor('[data-fieldtype="editor"]');
                    var buttons = editordiv.all('button');
                    var editorfield = editordiv.one('[contenteditable]');
                    if (disabled) {
                ...
                } else if (typeof M.editor_tinymce != 'undefined') {
                    M.editor_tinymce.toggle('id_' + name);
                }
                if (textareanode) {
                    if (disabled) {
        ...
        /**
         * Is the form element inside a editor
         *
         * @param {String} el The form element name.
         * @return {Boolean}
         */
        isEditor: function(el) {
            var editors = {};
            var textareafield = el + '[text]';
            var selector = '.fitem [data-fieldtype="editor"] textarea';
            var els = this.get('form').all(selector);
            els.each(function(node) {
                editors[node.getAttribute('name')] = true;
            });

            if (({}).hasOwnProperty.call(editors, textareafield)) {
                return editors[textareafield] || false;
            }
            return false;
        },
